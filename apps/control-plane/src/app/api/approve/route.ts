...
export async function POST(request: Request) {
  ...
  try {
    const run = await resolveApproval(body.approvalId, "approved", approver);
    await updateActionStatus(body.approvalId, "executed").catch(() => null);
    return NextResponse.json({ run, approver });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    if (message.includes("Permission Denied")) {
      return NextResponse.json({ error: message, approver }, { status: 403 });
    }
